...
export const PrimaryButton = styled(motion.a)<ButtonProps>`
  padding: ${({ padding }) => padding || `.4em 1em`};
  width: fit-content;
  font-family: inherit;
  border-radius: 0.2em;
  background-color: ${({ variant, theme }) =>
    variant === "primary" ? theme.foreground : theme.background};
  border: 2px solid ${({ theme }) => theme.foreground};
  color: ${({ variant, theme }) =>
    variant === "primary" ? theme.background : theme.foreground};
  cursor: pointer;
...
export const LevelTag = styled.span<TagProps>`
  ...
  background-color: ${({ level }) =>
    level === "Specific"
      ? `${CARDS.BROWN.backgroundColor}`
      : `${CARDS.GREEN.backgroundColor}`};
  border-radius: 0.2em;
  color: ${({ level }) =>
    level === "Specific"
      ? `${CARDS.BROWN.textColor}`
      : `${CARDS.GREEN.textColor}`};
  width: fit-content;
  height: fit-content;
  font-size: 1em;
`;
...
export const Notice = styled.section<NoticeProps>`
  ...
  background-color: ${({ variant }) =>
    variant === "Info"
      ? `${NoticeBackgroundColors.BLUE.background}`
      : `${NoticeBackgroundColors.RED.background}`};
  border-left: 3px solid
    ${({ variant }) =>
      variant === "Info"
        ? `${CARDS.BLUE.textColor}`
        : `${CARDS.RED.textColor}`};

  h4 {
    color: ${({ variant }) =>
      variant === "Info"
        ? `${CARDS.BLUE.headingColor}`
        : `${CARDS.RED.headingColor}`};
  }
`;
...
export const ContentInProgress = styled(Notice)`
  background-color: ${({ variant }) =>
    variant === "In Progress"
      ? `${NoticeBackgroundColors.BROWN.background}`
      : ""};
